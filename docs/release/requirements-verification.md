...
| V-162 | Confirmed reconciliation execution | `ConfirmedReconciliationRunner`, IPC Execute path, 20 Agent tests | implementation verified; real NTFS/non-NTFS run pending |
...
| V-167 | MFT performance matrix | `MftBenchmarks`, `Test-FullBenchmarkMatrix.ps1` | harness expanded; dedicated labeled volume/marker run pending |
