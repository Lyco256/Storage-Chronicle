...
| V-162 | Confirmed reconciliation execution | `ConfirmedReconciliationRunner`, IPC Execute path, bounded post-commit live-event buffer, 25 Agent tests | implementation verified; real NTFS/non-NTFS run pending |
...
| V-167 | MFT performance matrix | `MftBenchmarks`, `Test-FullBenchmarkMatrix.ps1` | actual MFT method harness exists, but required per-run dataset/candidate/detail-query/drop/environment oracle is not yet connected; dedicated labeled-volume run pending |
| V-168 | Physical installer acceptance | `build/package/New-ManualAcceptanceBundle.ps1`, `tools/PhysicalAcceptance/*`, installer build/test scripts and requirement 29 | physical install/repair/update/rollback not executed; manual bundle generation is implemented fail-closed and requires real base/updated/rollback MSI inputs |
