...
- physical installer and real Agent/Explorer correlation: not executed; the installer VM prerequisite matrix has no repository driver/orchestrator yet, and the physical driver intentionally remains a separate local-only path; the correlation wrapper intentionally reports fixture-only or live `NOT_EXECUTED` and has no synthetic Explorer substitute;
