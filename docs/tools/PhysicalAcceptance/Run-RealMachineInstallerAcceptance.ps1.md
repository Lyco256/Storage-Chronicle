...
Human-started runner for the physical Windows 10 22H2 or Windows 11 installer matrix. It verifies bundle hashes, target ProductName/DisplayVersion/build, x64, administrator token, an approved test-data root containing the TestLab `.storage-chronicle-testlab-marker.json`, and free space before asking for exact `YES` confirmation. It invokes the existing fail-closed installer matrix with the real case driver and writes a non-eligible preflight/failure artifact when the environment is wrong.
