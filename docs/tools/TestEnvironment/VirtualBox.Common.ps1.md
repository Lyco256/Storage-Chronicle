...
This is the provider-specific control boundary for the VirtualBox TestLab. It discovers and invokes the approved 7.2.x `VBoxManage.exe`, restricts VM identities and disk paths, enforces the 4096 MiB/2-vCPU/EFI/disabled-device profile, controls provisioning NAT versus acceptance-time offline state, restores `SC-CLEAN-BASELINE`, and transfers guest commands and artifacts through Guest Additions `guestcontrol`.
