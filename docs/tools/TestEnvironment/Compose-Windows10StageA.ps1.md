...
The script is an evidence composer; it does not run a VM, install software, change a volume, or create synthetic Stage A results. A passing artifact is therefore possible only after the real TestLab, privileged, installer, Cloud Files, and no-driver checks have run. The output is consumed by `tools/PhysicalAcceptance/Finalize-Windows10PhysicalAcceptance.ps1` and `build/quality/Test-FinalAcceptance.ps1`.
