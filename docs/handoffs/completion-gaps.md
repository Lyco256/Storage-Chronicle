...
- Non-privileged orchestration: all 22 test projects pass through `build/Test-Fast.ps1`; the MTP executable runner enforces at least one discovered test and the privileged project is not counted as a false-green zero-test run. The Agent project now has 20 passing tests, including IPC Execute delegation.
