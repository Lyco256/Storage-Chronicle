...
- R-03/R-19 audit found the existing resource gate lacked an independent lifecycle/timestamp-span/quiet-period evidence supervisor. Added `build/quality/Test-ResourceBudgetAcceptance.ps1`; it wraps the existing script without replacing it. The IPC client now uses cancellable asynchronous named-pipe I/O, and a current 15-second diagnostic with the actual Release Agent/Session Agent recorded complete queue sampling with zero missed samples; the required two-process 600-second acceptance run and quiet-period evidence remain pending.
