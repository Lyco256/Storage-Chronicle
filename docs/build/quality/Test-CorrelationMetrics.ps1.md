...
`-FixtureOnly` proves the deterministic fixture result but makes no live-machine claim. Without `-FixtureOnly`, the script runs the fixture, records its result, then marks the live Agent/interactive Session Agent measurement `NOT_EXECUTED` and exits `2`. A real TestLab/physical artifact may be supplied with `-LiveEvidencePath`; it must use `StorageChronicle.AgentExplorerCorrelationEvidence.v1`, set `AcceptanceEligible=true`, set `LiveMachineMeasurement=PASSED`, prove `FalseExactCount=0`, identify non-diagnostic `SC-Test-W11-VBox` TestLab execution, reference existing workload/oracle/history/executable files, and include internally consistent process, Explorer, and file/state correctness rows before the wrapper exits zero. This prevents an unexecuted machine measurement or a hand-written summary from being reported as a pass.

`-FixturePath` overrides the fixture path, `-OutputPath` selects the JSON report path, and `-LiveEvidencePath` supplies a real live-capture artifact after the fixture has run. The live artifact must reference existing executable/oracle/Explorer files and an Agent history directory, match the environment oracle path, contain positive durable source/canonical/final-state counts, an empty failure list, and expose nested `ProcessAttribution.FalseExactCount=0`. The script fails closed with exit `2` when the project, fixture, or report is missing; a test failure, a live artifact without the required zero-false-Exact proof, or a report that differs from the golden expectations exits nonzero as a failure.
