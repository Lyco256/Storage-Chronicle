...
public sealed class SettingsTests
{
    ...
    [Fact]
    public void WindowsPathProviderUsesRequiredMachineAndUserLocations()
    {
        var root = Path.Combine(Path.GetTempPath(), "StorageChroniclePathTests");
        var paths = new WindowsSettingsPathProvider(root, root);

        Assert.Equal(Path.Combine(root, "Storage Chronicle", "config", "machine-settings.json"), paths.MachineSettingsPath);
        Assert.Equal(Path.Combine(root, "Storage Chronicle", "user-settings.json"), paths.UserSettingsPath);
    }
