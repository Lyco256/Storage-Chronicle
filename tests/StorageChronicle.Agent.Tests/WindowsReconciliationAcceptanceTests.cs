    ...
    public async Task ConfirmedReconciliationAppendsARealDifferenceThroughTheProductionRunner()
    {
        ...
        try
        {
            ...
            await using (var storage = new AppendOnlyStorageEngine(new StorageEngineOptions(historyRoot) { FlushInterval = TimeSpan.FromMinutes(1) }))
            {
                ...
                WriteEvidence(evidencePath, new
                {
                    ...
                    MftEntryCount = summary.MftEntryCount,
                    ...
                    StartJournalId = summary.StartJournalBoundary?.JournalId,
                    StartJournalNextUsn = summary.StartJournalBoundary?.NextUsn,
                    CompletionJournalId = summary.CompletionJournalBoundary?.JournalId,
                    CompletionJournalNextUsn = summary.CompletionJournalBoundary?.NextUsn,
                    ScanCompletedUtc = summary.ScanCompletedUtc,
                    LiveEventCount = summary.LiveEventCount,
                    LiveEventsDeduplicated = summary.LiveEventsDeduplicated,
                    LiveEventsAccepted = summary.LiveEventsAccepted,
        ...
        catch (Exception exception)
        {
            WriteEvidence(evidencePath, new
            {
                ...
                MftEntryCount = summary?.MftEntryCount ?? 0,
                ...
                CompletionJournalNextUsn = summary?.CompletionJournalBoundary?.NextUsn,
                ScanCompletedUtc = summary?.ScanCompletedUtc,
                LiveEventCount = summary?.LiveEventCount ?? 0,
                LiveEventsDeduplicated = summary?.LiveEventsDeduplicated ?? 0,
                LiveEventsAccepted = summary?.LiveEventsAccepted ?? 0,
                StartedUtc = summary?.StartedUtc,
                FinishedUtc = summary?.FinishedUtc,
